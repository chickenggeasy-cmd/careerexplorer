...
> ไฟล์ `router.js` และ `quiz.js` มี comment อธิบายไว้ละเอียดที่สุด (เป็นจุดที่ logic ซับซ้อนกว่าไฟล์อื่น) แนะนำให้เปิดอ่าน 2 ไฟล์นี้ก่อนเพื่อเข้าใจโครงสร้างรวมของแอป ส่วนไฟล์อื่นยังอยู่ระหว่างทยอยเพิ่ม comment
...
| Accessibility / SEO | 🔴 จุดอ่อนที่สุด | คะแนนด้านนี้ต่ำกว่าด้านอื่นในทุกการประเมิน ยังไม่ได้ทำ semantic HTML, alt text, aria-label ครบทุกหน้า — ควรปรึกษารุ่นพี่ก่อนเริ่มแก้เพราะกระทบโครงสร้าง HTML หลายส่วน |
...
**ก่อนแก้ไขโค้ดที่ไม่เข้าใจ**
ตามนโยบายของทีม ให้ปรึกษารุ่นพี่ก่อนทุกครั้งที่ตัดสินใจเรื่องเทคนิคที่ไม่คุ้นเคย โดยเฉพาะเรื่อง Accessibility/SEO และการเปลี่ยนโครงสร้างข้อมูล (เช่นย้ายไปฐานข้อมูลจริง)
